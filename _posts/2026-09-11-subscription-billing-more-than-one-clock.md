...
2. **Resolution at evaluation.** The version is looked up at each calculation, using the calculation date. That's how I'm treating tax here, on purpose: nobody gets a grandfathered rate, the invoice date decides — real tax rules often depend on more than just a date. Strictly speaking, nothing is bound here, but "use the version in force at evaluation time" is still a decision.
...
`VersionInForceOn` only answers *when* — every trigger resolves to the same standard version for everyone on that date. A policy that instead preserves an already-earned benefit, or picks a version by cohort, would need to look at the customer too, not just the catalogue and the date.
...
(In practice, the tuple is resolved per evaluation period rather than per invoice: a mid-cycle tier change can give one invoice two periods, each with its own tuple — which assumes a history of bindings, not just the current one that `DiscountVersion` holds here. Resolving a period before the last trigger event needs effective-dated history this example leaves out.)
...
Worth naming what that move changes, not just where it lives: in the naive code a monthly customer just computed a zero discount, invoice unaffected. As a `Forbidden` rule, the same case blocks the binding outright at the next trigger event — a silent no-op becomes a blocked transition, and something now has to say what a monthly customer gets instead.
...
The compatibility catalogue has versions too. Recalculate Alice's September 2026 invoice in 2030 with the 2030 catalogue, and you may get a different amount. That's no longer a replay — it's a recalculation under a different policy context, useful for a simulation or correction, not a substitute for what was billed. An issued invoice never changes; replaying it with its own recorded versions is for checking it or preparing a credit note. So each invoice records its inputs and the versions it used:
...
Freezing a binding doesn't by itself make a forbidden tuple billable again — the billing-time check would otherwise catch it at the very next invoice. The exception has to carve that tuple out of the rule that forbids it, not just record that a decision happened; making that enforceable at billing time is a detail this model doesn't spell out.
...
Notice the check doesn't look at the rule's own verdict: a rule that newly makes a tuple `Allowed` needs a decision too, not just one that forbids something — though `DecisionKind` (Freeze/Migrate/Block) has no clean slot for "authorize," so the example stays scoped to rules that forbid. It also only sees tuples as they exist today: a tax version taking effect next month can't be gated on now, and the billing-time check from section 3 is what catches that case, a cycle later instead of before.
...
That's a real limit, not a detail I'm glossing over — and it runs deeper than the compatibility layer. A `CompatibilityRule` that only sees version numbers works well when compatibility is about which versions get along with each other. But tax here is resolved fresh at evaluation for everyone alike — it was never bound to anything per-subscription to begin with. Making it depend on when a subscription started isn't just a bigger tuple; it's a different binding policy for that axis, and the tuple isn't the only thing that would need to grow.
